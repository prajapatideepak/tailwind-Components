<template>
    <div class="bg-gray-200 h-full">
        <div role="alert" class="xl:w-5/12 mx-auto sm:mx-0 sm:w-6/12 md:w-3/5 w-11/12 bg-white dark:bg-gray-800 shadow-lg rounded flex sm:flex-row flex-col pr-4 absolute left-0 sm:left-auto right-0 sm:top-0 sm:mr-6 mt-16 sm:mt-6 mb-6 sm:mb-0 transition duration-150 ease-in-out" id="notification">
            <div class="sm:px-6 px-4 flex items-center sm:justify-center justify-start sm:border-r border-gray-300 dark:border-gray-700 mt-4 sm:mt-0 text-green-400">
            ...
            <div class="flex flex-col justify-center pl-4 py-4 sm:w-9/12">
                <p class="text-sm text-gray-800 dark:text-gray-100 font-semibold pb-1">Action Completed</p>
                <p class="text-xs text-gray-600 dark:text-gray-400 pb-2 font-normal">You have successfully read this alert message. Please check your email for further updates.</p>
                <div class="flex">
                    <span class="text-sm text-green-400 font-bold mr-2 cursor-pointer">Undo </span>
...
<script>
export default {
    name: "SimpleMultipleActionSuccess",
    mounted: function () {
        this.onLoad();
    },
    methods: {
